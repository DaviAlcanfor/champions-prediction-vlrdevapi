...
| Roster stability | Days with stable roster, new players in last 30 days |
| Playstyle | Aggressiveness, carry dependency, attack preference |
...
| Sample weights | Recent and important matches weighted higher |
...
**5. Run the pipeline**
```bash
python -m src.ingestion.kaggle_loader    # download datasets
python -m src.processing.normalizer      # normalize matches
python -m src.ingestion.vlr_api          # fetch team placements
python -m src.processing.placements      # process placements
python -m src.features.elo               # calculate Elo ratings
```
